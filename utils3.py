...
# return value, len, payload is bytes
...
# return value, len, payload in bytes
...
def base58decode(b):
    ...
    for i in range(0, len(b)):
        result = result * 58 + b58b.index(b[i])
    return result
...
def base58CheckEncode(version, payload):
    b = bytes((version,)) + payload
    b0 = hashlib.sha256(b).digest()
    checksum = hashlib.sha256(b0).digest()[0:4]
    result = b + checksum
    leadingZeros = countLeadingChars(result, 0)
    return b'1' * leadingZeros + base58encode(base256decode(result))

def base58CheckDecode(b):
    leadingOnes = countLeadingChars(b, 49)
    b0 = base58decode(b)
    b = base256encode(b0)
    result = b'\x00' * leadingOnes + b[:-4]
    chk = b[-4:]
...
class TestUtils(unittest.TestCase):
    ...
    def test_processAddr(self):
        self.assertEqual(processAddr(b'x'*20 + b'\x62\x91\x98\x16\x20\x8d'),
                         '98.145.152.22:8333')
    ...
    def test_base58(self):
        self.assertEqual(base58encode(base58decode(b'abc')), b'abc')
        self.assertEqual(base58decode(b'121'), 58)
        self.assertEqual(base58decode(b'5HueCGU8rMjxEXxiPuD5BDku4MkFqeZyd4dZ1jvhTVqvbTLvyTJ'),
    ...
    def test_base58check(self):
        ...
        self.assertEqual(base58CheckDecode(base58CheckEncode(0, b'\0\0abc')), b'\0\0abc')
        s = base256encode(0x0C28FCA386C7A227600B2FE50B7CAE11EC86D3BF1FBE471BE89827E19D72AA1D)
        b = base58CheckEncode(0x80, s)
        self.assertEqual(b, b"5HueCGU8rMjxEXxiPuD5BDku4MkFqeZyd4dZ1jvhTVqvbTLvyTJ")
