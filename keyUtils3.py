...
def privateKeyToWif(key_hex):    
    return utils3.base58CheckEncode(0x80, unhexlify(key_hex)).decode()

def wifToPrivateKey(s):
    b0 = s.encode('ascii')
    b = utils3.base58CheckDecode(b0)
    return b.hex()
...
def privateKeyToPublicKey(s):
    sk = ecdsa.SigningKey.from_string(unhexlify(s), curve=ecdsa.SECP256k1)
    vk = sk.verifying_key
    return (b'\04' + sk.verifying_key.to_string()).hex()

# Input is hex string
def keyToAddr(s):
    return pubKeyToAddr(privateKeyToPublicKey(s)).decode()

def pubKeyToAddr(s):
    ripemd160 = hashlib.new('ripemd160')
    ripemd160.update(hashlib.sha256(unhexlify(s)).digest())
    return utils3.base58CheckEncode(0, ripemd160.digest())

def addrHashToScriptPubKey(b58str):
    assert(len(b58str) == 34)
    # 76     A9      14 (20 bytes)                                 88             AC
    return '76a914' + utils3.base58CheckDecode(b58str).encode('hex') + '88ac'
...
class TestKey(unittest.TestCase):
    ...
    def test_pairs1(self):
        ...
        wallet_key = utils3.base256encode(utils3.base58decode(wallet_private)).hex()
        ...
        wallet_key = utils3.base58CheckDecode(bitcoin_qt).hex()
        self.assertEqual(keyToAddr(wallet_key), wallet_addr)
    ...
    def test_pairs3(self):
        ...
        self.assertEqual(keyToAddr(utils3.base58CheckDecode(wallet_private).hex()), wallet_addr)
    ...
    def test_derSigToHexSig(self):
        ...
        hashToSign = hashlib.sha256(hashlib.sha256(unhexlify(myTxn_forSig)).digest()).digest().hex()
        ...
        self.assertEqual(vk.verify_digest(unhexlify(sig), unhexlify(hashToSign)), True)
